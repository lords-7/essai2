...
then
  ...
  then
    ruby_FILENAME="portable-ruby-2.6.10.el_capitan.bottle.tar.gz"
    ruby_SHA="439a574381e3de7cd8f1772aa7e9b4a7f7e238fcf338b1b932573e9da0fee2b2"
  elif [[ "${HOMEBREW_PHYSICAL_PROCESSOR}" == "arm64" ]]
  then
    ruby_FILENAME="portable-ruby-2.6.10.arm64_big_sur.bottle.tar.gz"
    ruby_SHA="9bd0f46fb01911fee0c8aca85a876b165ef286afd4eba6b02d8e5db191cab7af"
  fi
elif [[ -n "${HOMEBREW_LINUX}" ]]
then
  case "${HOMEBREW_PROCESSOR}" in
    x86_64)
      ruby_FILENAME="portable-ruby-2.6.10.x86_64_linux.bottle.tar.gz"
      ruby_SHA="cabaab9e3dc97906795e8cf288c98c6420043dce0db5138669d97132da2d9c9d"
      ;;
    *) ;;
  esac
fi
...
then
  ...
  ruby_URLs+=(
    ...
    "https://github.com/Homebrew/homebrew-portable-ruby/releases/download/2.6.10/${ruby_FILENAME}"
  )
