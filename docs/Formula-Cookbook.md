...
* passes all `brew audit --formula --new <formula>` tests
...
New formulae being submitted to Homebrew should run `brew audit --formula --new foo`. This command is performed by Brew Test Bot on new submissions as part of the automated build and test process, and highlights more potential issues than the standard audit.
